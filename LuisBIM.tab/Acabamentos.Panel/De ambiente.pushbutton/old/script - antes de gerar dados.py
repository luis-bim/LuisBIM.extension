...
from System.Windows.Controls import (ComboBox, RadioButton, Button, TextBox,
                                     ...
                                     TextBlock, Label as WpfLabel) # WpfLabel para evitar conflito
...
class WpfConfigForm:
    ...
    def definir_padroes(self):
        ...
        self.txt_offset_value.Text = "0"  # Valor padrão do deslocamento
...
def create_walls_for_room(doc, room, wall_settings, creation_level_id, floor_settings=None):
    ...
    # **** LÓGICA DO DESLOCAMENTO SIMPLIFICADA ****
    base_offset_internal = 0.0
    if floor_settings:
        offset_cm = safe_get(floor_settings, "offset_cm", 0.0)
        base_offset_internal = offset_cm / 30.48 # Converte cm para pés (unidade interna)
    ...
    for wall_curve, host_wall in all_curves:
        try:
            ...
            if safe_get(wall_settings, "method") == "Valor":
                ...
                # Passa o deslocamento da base para a função de criação da parede
                ...
                # Cria a parede com o deslocamento e altura temporária
    ...
    if len(boundaries) > 0:
        ...
        if curveLoops.Count > 0:
            try:
                ...
                # Aplica o deslocamento do nível
                offset_cm = safe_get(floor_settings, "offset_cm", 0.0)
                internal_offset = offset_cm / 30.48 # Converte cm para pés
                ...
                try:
                    ...
                    for room in all_rooms:
                        ...
                        if safe_get(user_settings, "create_walls"):
                            # Passa as configurações de piso para a função de paredes, para que ela possa ler o valor do deslocamento
