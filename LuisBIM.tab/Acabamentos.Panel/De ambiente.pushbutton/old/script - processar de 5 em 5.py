...
clr.AddReference("System.Windows.Forms") # Adicionado para Windows Forms
clr.AddReference("System.Drawing")       # Adicionado para Windows Forms
    ...
    def ok_button_click(self, sender, e):
        ...
        if self.chk_create_walls.IsChecked:
            ...
            if self.radio_valor.IsChecked:
                ...
                try: output["wall_settings"]["value"] = float(self.txt_valor.Text); output["wall_settings"]["unit"] = self.combo_unidades.SelectedItem
...
# --- NOVA Janela de Progresso com Windows Forms (Leve) ---
class WinFormsProgressWindow:
    ...
    def update(self, current_val, total_val):
        ...
        Application.DoEvents() # Processa eventos da UI para manter a janela responsiva

    def interrupt_click(self, sender, e):
        self.interrupt = True
        self.win.Close()
...
def create_walls_for_room(doc, room, wall_settings):
    ...
    for wall_curve, host_wall in all_curves:
        try:
            ...
            if safe_get(wall_settings, "method") == "Valor":
                ...
                skirting = Wall.Create(doc, wall_curve, wall_type_id, room_level_id, height, 0, True, False)
            else:
                top_level_id = safe_get(wall_settings, "level_id")
                if top_level_id is None: continue
                skirting = Wall.Create(doc, wall_curve, wall_type_id, room_level_id, 1.0, 0, True, False)
...
try:
    ...
    floor_prefixes = ("ARQ-PISO-CERM", "ARQ-PISO-GNLT", "ARQ-PISO-CON", "ARQ-PISO-VIN", "ARQ-PISO-CARP")
    for t in floor_types:
        name = Element.Name.GetValue(t)
        if not t.IsFoundationSlab and name.startswith(floor_prefixes):
    ...
    if not wall_data['names'] and not floor_data['names']:
        ...
        if user_settings and (safe_get(user_settings, "create_walls") or safe_get(user_settings, "create_floors")):
            all_rooms = []
            try:
            ...
            if len(all_rooms) > 0:
                ...
                try:
                    ...
                    # Adicionado para otimização da barra de progresso
                    total_rooms = len(all_rooms)
                    for room in all_rooms:
                        counter += 1
                        
                        # >> OTIMIZAÇÃO: Atualiza a interface a cada 5 ambientes ou no último
                        ...
                        if safe_get(user_settings, "create_walls"):
                            walls_from_room = create_walls_for_room(doc, room, user_settings["wall_settings"])
